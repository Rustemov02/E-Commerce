        ...
        <Stack direction='row' flexWrap='wrap' justifyContent='center' gap={2}>
            ...
            {products.map((item, index) => (
                <Paper key={index} elevation={3} sx={styles.customPaper}>
                    ...
                    {isActive ? <Button onClick={()=>dispatch(getSelectedItem(item.id))} sx={styles.customButton} ><Link to={`/products/${item.id}`} style={{textDecoration : 'none' , color : 'black'}}>Buy Now</Link></Button> : <Skeleton variant="text" sx={{ height: 50, width: '30%' }} />}
